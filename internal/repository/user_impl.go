...
func (r *UserRepositoryImpl) CreateUser(balance float64) (*model.User, error) {
    var user model.User
...
func (r *UserRepositoryImpl) TransferMoney(fromUserID, toUserID int, amount float64) error {
    // Начинаем транзакцию
    tx, err := r.DB.Begin(context.Background())
    if err != nil {
        return fmt.Errorf("could not begin transaction: %v", err)
...
func (r *UserRepositoryImpl) TransferBalance(fromUserID, toUserID int, amount float64) (*model.User, *model.User, error) {
    tx, err := r.DB.Begin(context.Background())
    if err != nil {
